...
const updateSlidesProgress = function (translate = this.translate || 0) {
  const swiper = this
  const params = swiper.params
...
const updateProgress = function (translate = this.translate || 0) {
  const swiper = this
  const params = swiper.params
...
const updateSlidesClasses = function () {
  ...
  slides.removeClass(
    `${params.slideActiveClass} ${params.slideNextClass} ${params.slidePrevClass} ${params.slideDuplicateActiveClass} ${
...
const getTranslate = function (axis = this.isHorizontal() ? 'x' : 'y') {
  const swiper = this
...
const transitionStart = function (runCallbacks = true) {
  const swiper = this
...
const transitionEnd$1 = function (runCallbacks = true) {
  const swiper = this
...
const Browser = (function Browser() {
  ...
  return {
    ...
    ieTouch     :
      (win.navigator.msPointerEnabled && win.navigator.msMaxTouchPoints > 1) || (win.navigator.pointerEnabled && win.navigator.maxTouchPoints > 1),
    lteIE9: isIE9()
  }
}())

const slideTo = function (index$$1 = 0, speed = this.params.speed, runCallbacks = true, internal) {
  const swiper = this
...
const slideNext = function (speed = this.params.speed, runCallbacks = true, internal) {
  const swiper = this
...
/* eslint no-unused-vars: "off" */
const slidePrev = function (speed = this.params.speed, runCallbacks = true, internal) {
  const swiper = this
  const { params, animating } = swiper
...
/* eslint no-unused-vars: "off" */
const slideReset = function (speed = this.params.speed, runCallbacks = true, internal) {
  const swiper = this
...
const onResize = function () {
  ...
  const {
    params, el, allowSlideNext, allowSlidePrev
  } = swiper
...
const Virtual = {
  update(force) {
    ...
    const {
      from: previousFrom, to: previousTo, slides, slidesGrid: previousSlidesGrid, renderSlide, offset: previousOffset
    } = swiper.virtual
    swiper.updateActiveIndex()
...
const Pagination = {
  update() {
    ...
    if (params.type === 'bullets' && swiper.pagination.bullets && swiper.pagination.bullets.length > 0) {
      ...
      bullets.removeClass(
        `${params.bulletActiveClass} ${params.bulletActiveClass}-next ${params.bulletActiveClass}-next-next ${params.bulletActiveClass}-prev ${
